...
const TambahMakanan = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validateForm()) {
      console.log('Data makanan:', formData);
  ...
  return (
    <div className="w-full h-full p-6 overflow-auto">
      <div className="max-w-3xl mx-auto">
        <div className="mb-5">
          <h1 
            className="text-3xl text-[#1F2D3B] tracking-wide"
            style={{ fontFamily: 'Bowlby One, cursive', fontWeight: 'normal' }}
          >
            Tambah Makanan
          </h1>
        </div>

        <div className="bg-gradient-to-b from-[#FFE4C7] to-white rounded-2xl shadow-md overflow-hidden">
          <form onSubmit={handleSubmit} className="px-7 py-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
              ...
              <div>
                ...
                <div className="relative" ref={dropdownRef}>
                  <div 
                    className={`w-full px-4 py-2.5 bg-white border rounded-xl flex items-center justify-between cursor-pointer transition-all ${
                      errors.kategori ? 'border-red-400' : 'border-gray-300'
                    } hover:border-[#2F5D56]`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`w-full px-4 py-2.5 bg-white border rounded-xl outline-none transition-all focus:border-[#2F5D56] text-sm ${
                    errors.tanggalBeli ? 'border-red-400' : 'border-gray-300'
                  }`}
                  style={{ fontFamily: 'Jomolhari, serif', fontWeight: 'normal' }}
                />
            ...
            <div className="mb-7">
              ...
              <div className="relative" ref={satuanDropdownRef}>
                <div 
                  className={`w-full px-4 py-2.5 bg-white border rounded-xl flex items-center justify-between cursor-pointer transition-all ${
                    errors.satuan ? 'border-red-400' : 'border-gray-300'
                  } hover:border-[#2F5D56]`}
                ...
                {isSatuanDropdownOpen && (
                  <div className="absolute z-20 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
                    {satuanOptions.map((option) => (
                      <div
                        key={option.value}
                        className="px-4 py-2.5 hover:bg-[#FFE4C7] cursor-pointer transition-colors text-sm"
                        style={{ fontFamily: 'Jomolhari, serif', fontWeight: 'normal' }}
